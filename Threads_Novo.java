...
import java.util.ArrayList;
		...
		class Threads_Novo implements Runnable{
			int index=0;
			int thread=0;
			String alignTool="";
			String homePath="";
			String filePath="";
			String kmerFile="";
			public Threads_Novo(int Index, String HomePath, String FilePath, String KmerFile, int Threads)
			{
				index=Index;
				thread=Threads;
				filePath=FilePath;
			...
			public void run() {
				...
				{   
					...
					String[] cmd_Index = { "sh", "-c", homePath+"/tools/bowtie2-bin/bowtie2-build "+homePath+"/alignment/read_"+index+".fa "+homePath+"/alignment/fre_Ref_"+index};
					pr_index=r_index.exec(cmd_Index);
					pr_index.waitFor();
				    Runtime r_mapping = Runtime.getRuntime();
				    Process pr_mapping=null;	    
				    String[] cmd_mapping = { "sh", "-c", homePath+"/tools/bowtie2-bin/bowtie2 -x "+homePath+"/alignment/fre_Ref_"+index+" -f -a -p 32 -U "+homePath+"/readfile/KmerFile.fa -S "+homePath+"/alignment/read_"+index+".sam"};
					...
					if (SamFilePath.isFile() && SamFilePath.exists()) {
					 ...
					 inChanne2.close();
					 aFile2.close();
					 CommonClass.DelePathFiles(homePath+"/alignment/","read_"+index+".sam");
